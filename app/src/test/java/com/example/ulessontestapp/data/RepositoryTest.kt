...
class RepositoryTest {
    ...
    fun `assert that repository saves recent view`() = mainCoroutineRule.runBlockingTest {
        val argumentCapture = argumentCaptor<RecentlyWatched>()

        repository.saveRecentWatch(TestObjectUtil.recentViews[0])
        verify(localDataSource).saveRecentWatch(argumentCapture.capture())
    ...
    fun `assert that repository retrieves recent views`() {
        val data = MutableLiveData<List<RecentlyWatched>>()
        data.value = TestObjectUtil.recentViews.take(2)

        `when`(localDataSource.observeRecentWatch(2)).thenReturn(data)

        val response = repository.getRecentWatch(2).getOrAwaitValue()
        verify(localDataSource).observeRecentWatch(2)
